...
const SECTIONS = [
  ...
  { id: 'library', label: 'WhutBux?', icon: 'M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z' },
...
// File Manager — admin-only seedbox file browser.
const FILE_MANAGER_SECTION = { id: 'file-manager', label: 'File Manager', icon: 'M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z' };
...
export default function Sidebar({ section, onNavigate, collapsed, onToggleCollapse, onLogout, isAdmin, allowedViews }) {
  // Per-user view authorization. null/undefined = all allowed (admins, or while
  // permissions are still loading); otherwise only ids in the array are shown.
  const canView = (id) => !allowedViews || allowedViews.includes(id);
  const visibleSections = SECTIONS.filter((s) => canView(s.id));
      ...
      <nav className="flex-1 py-4 px-3 space-y-1">
        ...
        {isAdmin && (
          <>
            ...
            <button
              onClick={() => onNavigate(FILE_MANAGER_SECTION.id)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 group
                ${section === FILE_MANAGER_SECTION.id
                  ? 'bg-vault-accent/15 text-vault-accent'
                  : 'text-vault-muted hover:text-vault-text hover:bg-vault-card'
                }`}
            >
              <svg className="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d={FILE_MANAGER_SECTION.icon} />
              </svg>
              {!collapsed && (
                <>
                  <span>{FILE_MANAGER_SECTION.label}</span>
